...
This document is a chronological research trail, not a current-state summary.
Each "Addendum" below is a snapshot of what was true, tried, or found at that
specific session -- read from the bottom up for the oldest material, or from
the top down for the most recent. Nothing here is meant to describe the
project's current state as a whole.

**For the actual current state** (what's confirmed, what's still open, the
per-feature confidence breakdown), see `README.md`'s "Confidence & known gaps"
section -- that's the one place this project keeps deliberately current, not
this document. (A hand-maintained "current state" summary used to live here
instead; it drifted to describe the project as of the ninth session while the
document grew past the fifty-eighth -- removed rather than rewritten, since a
second, separately-maintained summary is exactly the kind of thing that
reliably goes stale. One source of truth for "what's true now," this document
for "how we got here and why.")
...
[`base_roomba_config_REFERENCE.json`](base_roomba_config_REFERENCE.json)) --
...
- **Ader update draft**: `../archive/ADER_UPDATE_DRAFT_2026-07-11.md` --
...
(`CHANGELOG.md`, this document, `DEVELOPMENT_NOTES.md`'s dated confidence tables) that
