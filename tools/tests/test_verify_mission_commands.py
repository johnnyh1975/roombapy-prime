...
confirm()'s and _run_command()'s logic, completely mocked (real robot,
...
from roombapy_prime_tools.verify_mission_commands import (
...
def test_confirm_only_accepts_explicit_affirmatives(answer, expected, monkeypatch) -> None:
    ...
    assert confirm("Test?") is expected
...
async def test_run_command_sends_and_records_success(monkeypatch) -> None:
    ...
    monkeypatch.setattr("roombapy_prime_tools.verify_mission_commands.asyncio.sleep", AsyncMock())
