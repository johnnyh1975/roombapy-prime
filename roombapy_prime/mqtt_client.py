...
# EXPERIMENTAL, UNCONFIRMED FOR THIS SPECIFIC APP (this session): the
# WebSocket upgrade for the MQTT connection previously sent NO
# User-Agent header at all. An independent project reverse-engineering
# a DIFFERENT (but related) iRobot app documented -- without a
# reproducible test tool to verify it, so treat the REASONING as
# unconfirmed too -- that AWS IoT's custom Lambda authorizer inspects
# this specific header and grants a more restricted IoT policy when
# it's absent.
#
# The VALUE below ("?SDK=Android&Version=2.17.1") is THEIRS, confirmed
# in their own working code (not just docs) -- but it's for a
# different app entirely, so copying it isn't "impersonating the real
# Prime app" the way copying an app-specific identifier (like app_id)
# would be a bad idea. This specific format (?SDK=<platform>&Version=
# <version>) looks like it's generated by the AWS IoT Android SDK
# itself, not customized per-app -- if true, many apps built on the
# same/similar SDK version could plausibly send an identical string
# regardless of which app it is, making this a more defensible test
# candidate than blindly copying something app-specific would be.
# Still unconfirmed for Prime specifically -- a real APK search for
# this exact pattern (or the SDK version Prime's app actually bundles)
# would settle it properly.
_TEST_CANDIDATE_USER_AGENT = "?SDK=Android&Version=2.17.1"
...
class PrimeMqttClient:
    ...
    def _build_client(self) -> mqtt.Client:
        ...
        client.ws_set_options(
            ...
            headers={
                ...
                # NEW (this session) -- see _TEST_CANDIDATE_USER_AGENT's own
                # docstring for why this specific value and why it's still
                # marked experimental/unconfirmed for Prime specifically.
                "User-Agent": _TEST_CANDIDATE_USER_AGENT,
            },
        )
        try:
            import certifi
    ...
    def _on_subscribe(self, client, userdata, mid, reason_codes, properties=None) -> None:
        ...
        failed_codes = [rc for rc in reason_codes if int(rc) >= 0x80]
        if failed_codes:
    ...
    def rejected_report_topic(self, irbt_topic_prefix: str) -> str:
        ...
        mission_timeline_topic()'s own docstring), payload shape
        unknown."""
    ...
    def publish_cmd(self, irbt_topic_prefix: str, command: str, initiator: str = "localApp") -> bool:
        ...
        session) -- see publish_cmd_payload()'s own docstring for the
