...
class RegionType(str, Enum):
    """Bestaetigt (androguard): nur 3 Werte. RID/TID/ZID vermutlich Room-
    ID/Track-ID(?)/Zone-ID -- Bedeutung von TID nicht sicher, aber der
    Name selbst ist bytecode-bestaetigt."""
...
class CommandParams:
    ...
    def to_json(self) -> dict[str, Any]:
        ...
        raw = {
            ...
            "cleaningProfile": self.cleaning_profile,
    ...
    def from_json(cls, data: dict[str, Any]) -> CommandParams:
        ...
        return cls(
            ...
            cleaning_profile=data.get("cleaningProfile"),
...
class DoneCode(str, Enum):
    """Bestaetigt (androguard): 19 Werte -- der Beendigungsgrund einer
    Mission. Sehr nuetzlich fuer Auswertung der Missionshistorie."""
...
class MissionCommandRecord:
    """Bestaetigt (androguard, MissionCommand -- Missionshistorie-
    Variante, NICHT zu verwechseln mit RoutineCommand): cleanAll,
    command (String, nicht MissionCommandType-Enum -- lockerer typisiert
    als der Sende-Pfad), initiator, mapId, mapVersionId, ordered,
    regions (rohe Liste, Struktur nicht weiter untersucht), robotId,
    time."""
    ...
    def from_json(cls, data: dict[str, Any]) -> MissionCommandRecord:
        return cls(
            ...
            map_version_id=data.get("mapVersionId"),
...
class MissionHistoryEntry:
    ...
    def from_json(cls, data: dict[str, Any]) -> MissionHistoryEntry:
        ...
        timeline_events = timeline_data.get("events") if isinstance(timeline_data, dict) else timeline_data
        return cls(
            ...
            minutes_running=data.get("minutesRunning"),
            minutes_paused=data.get("minutesPaused"),
            minutes_charging=data.get("minutesCharging"),
            minutes_done=data.get("minutesDone"),
            done_code=_enum_or_none(DoneCode, data.get("doneCode")),
            done_raw=data.get("doneRaw"),
            error_code=data.get("errorCode"),
            square_feet_covered=data.get("squareFeetCovered"),
            number_of_evacuations=data.get("numberOfEvacuations"),
            number_of_dirt_detects=data.get("numberOfDirtDetects"),
            docked_at_start=data.get("dockedAtStart"),
            ended_on_dock=data.get("endedOnDock"),
...
class TravelDestination(str, Enum):
    """Bestaetigt (androguard, TravelEvent.destination): 5 Werte."""
...
class TraversalType(str, Enum):
    """Bestaetigt (androguard, TraversalEvent.type): 2 Werte."""
...
@dataclass(frozen=True)
class PadWashEvent:
    """Bestaetigt (jadx): error, fluidAmount, padWashState, reason --
    Wischpad-Waschzyklus."""
    ...
    def from_json(cls, data: dict[str, Any]) -> PadWashEvent:
        return cls(
            ...
            pad_wash_state=data.get("padWashState"),
...
@dataclass(frozen=True)
class RoomEvent:
    """Bestaetigt (jadx): area, conPasses, mapId, mapVersion, passArea,
    passCount, regionId, status, totalArea -- Fortschritt pro Raum."""
...
class TentativeLocationEvent:
    """Bestaetigt (jadx): confirmedMapId, confirmedMapVersion,
    confirmedRegionId, mapId, mapVersion, regionId. WICHTIG: wird auf
    MissionTimelineEvent unter ZWEI verschiedenen Feldnamen referenziert
    ("relocalizing" UND "tentativeLocation") -- derselbe Typ, zwei
    Rollen (vermutlich: waehrend der Neulokalisierung vs. danach
    bestaetigt)."""
    ...
    def from_json(cls, data: dict[str, Any]) -> TentativeLocationEvent:
        return cls(
            confirmed_map_id=data.get("confirmedMapId"),
            confirmed_map_version=data.get("confirmedMapVersion"),
            confirmed_region_id=data.get("confirmedRegionId"),
            map_id=data.get("mapId"),
            map_version=data.get("mapVersion"),
            region_id=data.get("regionId"),
        )
...
@dataclass(frozen=True)
class TravelEvent:
    """Bestaetigt (androguard): destination (TravelDestination), mapId,
    mapVersion, polyId, reason, regionId, status, waypointId, zoneId."""
    ...
    def from_json(cls, data: dict[str, Any]) -> TravelEvent:
        return cls(
            destination=_enum_or_none(TravelDestination, data.get("destination")),
            map_id=data.get("mapId"),
            map_version=data.get("mapVersion"),
            poly_id=data.get("polyId"),
            reason=data.get("reason"),
...
class TraversalEvent:
    """Bestaetigt (androguard): mapId, mapVersion, regionId, type
    (TraversalType), zoneId."""
...
@dataclass(frozen=True)
class ZoneEvent:
    """Bestaetigt (jadx): area, mapId, mapVersion, passArea, passCount,
    status, totalArea, zoneId -- Fortschritt pro Zone."""
...
class MissionTimelineEvent:
    ...
    def from_json(cls, data: dict[str, Any]) -> MissionTimelineEvent:
        ...
        return cls(
            ...
            relocalizing=_sub("relocalizing", TentativeLocationEvent.from_json),
